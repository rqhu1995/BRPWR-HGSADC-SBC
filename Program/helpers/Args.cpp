...
void parseArgs(int argc, char *argv[]) {
  ...
  parser.set_optional<double>("tb", "time_budget", -1,
                              "time budget for the repositioning");

  // ==== params related to the algorithm ====
  parser.set_optional<int>(
      "pnt", "penalty", 10,
      "initial penalty for each violation of the constraints");
  parser.set_optional<int>("noimp", "num_non_improve", 5000,
                           "number of iterations without improvement");
  parser.set_optional<double>("bprop", "broken_proportion", -0.5,
                              "broken bike proportion");
  parser.set_optional<int>("num_pnt_manage", "num_penalty_management", 100,
                           "number of iterations for penalty management");
  parser.set_optional<double>(
      "fesP", "target_feasible", 0.2,
      "target feasible solution percentage in the population");
  parser.set_optional<double>(
      "tl", "timeLimit", 7200,
      "time limit for the algorithm as one of the terminating criteria");
  parser.set_optional<int>("mu", "mu", 25, "mu");
  parser.set_optional<int>("lambda", "lambda", 40, "lambda");
  parser.set_optional<int>(
      "edu", "itedu", 40,
      "number of iterations repeated when an operator improved a solution");
  ...
  // if timeBudget is not provided, then timeBudget is set based on the number
  // of stations, the mapping is as follows: 6, 10, or 15: 7200; 20,30: 10800;
  // 60,120: 14400; 200,300,400,500: 18000
