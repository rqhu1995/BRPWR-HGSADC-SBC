...
// Constructor
LocalSearch::LocalSearch(Params &params, Instance &instance) : params(params), instance(instance) {
}
...
void LocalSearch::run(Individual &indiv) {
    ...
    std::vector<std::pair<EduOp, std::string>> move_list
        = {std::make_pair([this](Individual &indiv) { return this->move_1(indiv); }, "move_1"),
            std::make_pair([this](Individual &indiv) { return this->move_2(indiv); }, "move_2"),
            std::make_pair([this](Individual &indiv) { return this->move_3(indiv); }, "move_3"),
            std::make_pair([this](Individual &indiv) { return this->move_4(indiv); }, "move_4"),
            std::make_pair([this](Individual &indiv) { return this->move_5(indiv); }, "move_5"),
            std::make_pair([this](Individual &indiv) { return this->move_6(indiv); }, "move_6"),
            std::make_pair([this](Individual &indiv) { return this->move_7(indiv); }, "move_7"),
            std::make_pair([this](Individual &indiv) { return this->move_8(indiv); }, "move_8"),
            std::make_pair([this](Individual &indiv) { return this->move_9(indiv); }, "move_9")};
    while (!move_list.empty()) {
        ...
        Individual educated_ind = executeAndDisplayMove(edu_op, indiv, move_list[index].second);
...
void LocalSearch::swapTwoRandomElements(std::vector<std::vector<int>> &routes) {
    for (auto &route : routes) {
        if (route.size() > 3) {
            int index1 = getRand(route.size() - 2) + 1; // Adjusted to exclude depot nodes
            int index2 = getRand(route.size() - 2) + 1; // Adjusted to exclude depot nodes
            while (index1 == index2) {
                index2 = getRand(route.size() - 2) + 1; // Adjusted to exclude depot nodes
            }
...
void LocalSearch::swapTwoSubtours(std::vector<std::vector<int>> &routes) {
    for (auto &route : routes) {
        if (route.size() > 4) {                         // Ensure there are more than 3 nodes excluding depot nodes
            int index1 = getRand(route.size() - 2) + 1; // Adjusted to exclude depot nodes
            int index2 = getRand(route.size() - 2) + 1; // Adjusted to exclude depot nodes
            int index3 = getRand(route.size() - 2) + 1; // Adjusted to exclude depot nodes
            int index4 = getRand(route.size() - 2) + 1; // Adjusted to exclude depot nodes
            ...
            newRoute.insert(newRoute.end(), route.begin(), route.begin() + indices[0]);

            // Insert the second subtour
            newRoute.insert(newRoute.end(), route.begin() + indices[2], route.begin() + indices[3] + 1);

            // Insert the part between the two subtours
            newRoute.insert(newRoute.end(), route.begin() + indices[1] + 1, route.begin() + indices[2]);

            // Insert the first subtour
            newRoute.insert(newRoute.end(), route.begin() + indices[0], route.begin() + indices[1] + 1);

            // Insert the part after the second subtour
            newRoute.insert(newRoute.end(), route.begin() + indices[3] + 1, route.end());
    ...
    for (auto &route : routes) {
        if (route.size() > 3) {
            ...
            if (route.size() == 2) {
                ...
                do { index2 = getRand(route.size() - 1) + 1; } while (index2 == index1 && route.size() > 3);
            }
...
void LocalSearch::relocateOneSubtour(std::vector<std::vector<int>> &routes) {
    for (auto &route : routes) {
        ...
        {
            int index1 = getRand(route.size() - 3) + 1; // Adjusted to exclude depot nodes
            int index2 = getRand(route.size() - 3) + 1; // Adjusted to exclude depot nodes
            while (index1 == index2) {
                index2 = getRand(route.size() - 3) + 1; // Adjusted to exclude depot nodes
            }
            if (index1 > index2) { std::swap(index1, index2); }
            std::vector<int> subroute(route.begin() + index1, route.begin() + index2 + 1);
            route.erase(route.begin() + index1, route.begin() + index2 + 1);

            int index3;
            if (route.size() > 2) {                     // Adjusted to account for depot nodes
                index3 = getRand(route.size() - 2) + 1; // Adjusted to exclude depot nodes
            } else {
...
void LocalSearch::insertOneNode(std::vector<std::vector<int>> &routes, bool isRPM) {
    for (auto &route : routes) {
        // check the number of different node in the route
        std::unordered_set<int> nodeSet;
        for (auto node : route) {
            if (node != 0) { nodeSet.insert(node); }
        }
        if (nodeSet.size() == params.nbClients && isRPM) { continue; }
        if (route.size() > 3) {
            int index1 = getRand(route.size() - 2) + 1; // Adjusted to exclude depot nodes
...
void LocalSearch::insertMultipleNodes(std::vector<std::vector<int>> &routes, bool isRPM) {
    for (auto &route : routes) {
        if (route.size() >= 3) {
            // generate the number of nodes to insert
            int numNodes = getRand(route.size() - 2) + 1;
            // call insertOneNode for numNodes times
            for (int i = 0; i < numNodes; i++) { insertOneNode(routes, isRPM); }
        }
    }
}
...
void LocalSearch::deleteMultipleNodes(std::vector<std::vector<int>> &routes) {
    for (auto &route : routes) {
        ...
        if (route.size() >= 3) {
            ...
            for (int i = 0; i < numNodes; i++) { deleteOneNode(routes); }
        }
    }
}
...
Individual LocalSearch::move_9(Individual &indiv) {
    // Extract chromRPM and chromTRK from indiv
    std::vector<std::vector<int>> chromRPM = indiv.chromRPM;
    std::vector<std::vector<int>> chromTRK = indiv.chromTRK; // Delete multiple nodes in chromRPM and chromTRK
...
void LocalSearch::display2DVector(const std::vector<std::vector<int>> &routes) {
    for (const auto &route : routes) {
        for (const auto &node : route) { std::cout << node << " "; }
        std::cout << std::endl;
    }
}

Individual LocalSearch::executeAndDisplayMove(
    const std::function<Individual(Individual &)> &moveFunc, Individual &indiv, const std::string &moveId) {
    // Extract chromRPM and chromTRK from indiv
    std::vector<std::vector<int>> chromRPM = indiv.chromRPM;
    std::vector<std::vector<int>> chromTRK = indiv.chromTRK;
